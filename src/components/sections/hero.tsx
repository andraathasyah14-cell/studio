  ...
  return (
    <section id="home" className="py-24 px-6 border-b border-border grid grid-cols-1 md:grid-cols-5 gap-16 items-end">
      ...
      <div className="md:col-span-2 space-y-px">
        <StatRow label="Tulisan" value={stats.essays} />
        <StatRow label="Paper digunakan" value={stats.papers} />
        <StatRow label="Buku dibaca" value={stats.books} />
        <StatRow label="Revisi tulisan" value={stats.revisions} />
        <StatRow label="Opini ditarik" value={stats.retractions} />
      </div>
    </section>
  );
}
